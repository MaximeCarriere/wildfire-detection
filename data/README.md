...
## D-Fire (present on disk): train, val and in-distribution test
...
Observed across all 21,527 images: **class 0 gives 11,865 boxes, class 1 gives 14,692**,
...
D-Fire's **published train/test split is preserved**, because it is the split every paper
reports against and re-shuffling it would silently cost that comparison. Only *val* is carved out,
...
well-populated metric rather than a noisy one, which matters because early detection of
distant smoke is the hardest part of this problem.

## Out-of-distribution sets: test only, quarantined (not yet on disk)
...
| Boreal Forest Fire (~1k sampled) | optional second OOD, European vegetation | ⬜ optional | Scientific Data release |
...
The set is built to cover night, fog, sunset/backlight and small distant plumes, which are
...
(Ultralytics); see the licence note in the top-level README.
